...
**R-11 — Flywheel de calidad (PLANIFICADO 19-ago-2026, NO implementado)**: capa transversal de mejora continua sobre R-09/R-10. Ciclo: `uso real → telemetría → agregación → insight → ticket → aprobación staff → contenido mejorado`. Plan completo en `docs/plan-r11.md`. **Hallazgos de auditoría**: el reporte asumía piezas inexistentes — `assessmentGenerator.ts`→real es `matchScorer.ts`+`stage1Service.ts`, `interviewEngine.ts`→real es `interview.ts`, `learningAnalytics.ts` y `piiScrubber.ts` son nuevos. Modelo: migración `20260819020000_quality_flywheel.sql` (sim_events, item_stats, misconceptions, improvement_tickets, outcome_tracking, RLS, user_hash irreversible, cero PII). Servicios: `learningAnalytics.ts` (ingesta batch/agregación/misconceptions/tickets por umbrales fail_rate>0.7, gain<0.2), `piiScrubber.ts`. Endpoints: `POST /api/sim/telemetry`, `GET /api/staff/quality`, `GET /api/staff/tickets`, `POST /api/staff/tickets/:id/approve|reject`, `DELETE /api/staff/users/:hash/data`. Consumo (T5): `taskPlanner` (dificultad+drills), `caseGenerator` (trampas de misconceptions, pasa story-coherence), `stage1Service`/`matchScorer` (filtrar preguntas), `deValidation`/`workflowEngine` (feedback), `interview.ts` (preguntas por errores reales), `careerPath`+`vacancyAnalyzer` (coverage_gap). Staff: `QualityPanel.tsx` como 4ª sección de `StaffControlCenter`. Tareas T1-T7. Tests `tests/analytics.test.ts` (agregaciones, ticket con fail_rate>0.7 & gain<0.2, cero PII, nada sin aprobación staff, trampa pasa story-coherence). Regla de oro extendida: números/umbrales de motores o agregación de datos reales; nada se despliega sin gate staff + story-coherence.
