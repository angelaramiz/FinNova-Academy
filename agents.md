...
6. **TDD obligatorio para todo lo nuevo**: primero el test en rojo (falla), luego el código mínimo que lo pone en verde, luego refactor. Sin excepciones para features, fixes, endpoints, validadores, workflows o migraciones. Todo código nuevo entra con su `tests/*.test.ts` (o `backend/src/**/*.test.ts`) y `npm run test` + `npm run audit:story` (cuando aplique) en verde antes de dar la tarea por cerrada.
...
enderTool para stats/ml/monitor; SpreadsheetSim gana XLOOKUP/BUSCARX/SUMIFS/COUNTIFS/UNIQUE/FILTER. docs/legacy/simulab-v2-pausado/simulab-v2-guess-encargado-almacen.json (SIMULAB v2: c/buildPlan densidad 0.4/0.2/0.15/0.15/0.1, cierre por criterio, oral 5m; Bloque 6 VBA→Modelo de Datos Power Pivot/DAX). docs/legacy/planes-implementados/plan-capas-0-ecosistema.md IMPLEMENTADO (9 gaps resueltos). docs/legacy/qa-historico/qa-capa0-2026-08-26.md (plan QA en producción con Chrome DevTools, bloques A-H + ejecución y re-validación consolidadas). Tests 	ests/capa-zero.test.ts (6). Suite 301 tests / audit 106 / builds limpios. Verificado prod: 	ask-plan?route=analyst lista fundamentos→ecosistema; excel_basico/i_basico validan 10/10.
