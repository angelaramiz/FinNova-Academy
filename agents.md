...
**R-13 — Prácticas Profesionales: ruta guiada de contabilidad (20-ago-2026)**: ruta **nueva** `practicas` para alumnos reales de contabilidad que usan el simulador como prácticas profesionales, con **guía pedagógica por módulos y procedural** (burbujas que explican qué se hace en cada portal/hoja y cómo registrar). Backend: `workflowEngine.ts` gana interfaz `GuideBubble` (`{id,title,body,anchor?,position?}`) y campo `WorkflowStep.guides`; guías agregadas a `invoice_emission` (portal SAT, CFDI 4.0, RFC exacto, IVA 16%, uso CFDI, trampa IVA 10%), `supplier_invoice` (IVA acreditable) y `payment_registration` (aplicación de pago, saldo pendiente); nuevo workflow `business_expense` (gasto por comida empresarial: email → spreadsheet → result; filas Subtotal/Propina no deducible/IVA 16%/Total/Gasto deducible 65% restaurantes/IVA acreditable; validación por `workflowId` con labels `IVA del consumo`, `Total pagado`, `Gasto deducible`, `IVA acreditable`) registrado en `accountingTypes` del router `workflows.ts`; `generateBusinessExpenseEntries` en `autoEntries.ts` (asiento: cargo `5-03 Gastos de administración` subtotal + cargo `2-03 IVA por pagar` + cargo `5-08 Gastos no deducibles` propina / abono `1-02 Bancos` total — cuadra débitos=créditos). `practicasModules.ts` (NUEVO): catálogo de 6 módulos con pasos procedurales (`mod-cfdi`, `mod-gastos`, `mod-cobranza`, `mod-proveedores`, `mod-nomina`, `mod-cierre`) + `auditPracticasModules`. Endpoints `GET /api/sim/practicas/modules` y `GET /api/sim/practicas/modules/:id` en `simEngine.ts` + case `business_expense` en `/generate-entries`. `taskPlanner.ts`: template `business_expense`, `PRACTICAS_WEEKS` (4 semanas por módulos 1-6) y `generateMonthPlan` usa PRACTICAS_WEEKS cuando `specialtyId === 'practicas'`. `specialties.ts`: `business_expense` en `workflowTypes` contables. Frontend: `GuideBubbles.tsx` (NUEVO, botón flotante "💡 Guía" + barra de progreso + Anterior/Siguiente + anillo de resaltado anclado por selector CSS); `PracticasModules.tsx` (NUEVO, currículum de módulos con progreso en localStorage + detalle procedural + botón "▶ Abrir tarea"); `DesktopShell.tsx` (Screen `'practicas'`, `appSet='practicas'`, `practicasApps` con "📚 Módulos", banner 🎓, render de `<GuideBubbles>` en cada paso del workflow, `generateEntriesForTask` mapea `business_expense` desde filas del spreadsheet); `AccountingForm.tsx` agrega `data-guide={field.key}` por campo; `SpreadsheetWidget.tsx` agrega `data-guide={row.label}` por fila; `Onboarding.tsx` nueva especialidad "🎓 Prácticas Profesionales" (job "Practicante de Contabilidad"); `SimuladorLaboral.tsx` propaga `specialty='practicas'` como contable (banner propio, sin tocar rutas data). Tests `tests/practicas-modules.test.ts` (15: guías en workflows, estructura/validación del gasto — propina no deducible, IVA acreditable, asiento cuadra — catálogo 6 módulos, cada paso referencia workflow real, specialty practicas genera tareas de módulos). Verificado end-to-end vía API: módulos 200, workflow `business_expense` 100% con `workflowId`, `today-tasks` semana 2 = business_expense. Gates: root **252** tests / audit **106** / backend tsc / builds alumnos y staff limpios.

**R-13.5 — Tracker semanal, prueba por tema y curso teórico con NPC capacitador (20-ago-2026)**: sobre R-13, convierte las prácticas en un programa de mecanización + comprensión. `practicasModules.ts` ampliado: cada módulo gana `prueba: PracticaPrueba` (título, `aprobarMin`, preguntas con opciones/índice correcto/explicación pedagógica) y `curso: PracticaCurso` (introducción/secciones/cierre narrados por el NPC `capacitador`); `PRUEBAS` y `CURSOS` por módulo (CFDI/IVA 16% no 10%, propina no deducible, 65% restaurantes, ISR por tabla no % fijo, conciliación). `buildPracticasTracker(month, year)` deriva las 4 semanas del PLAN REAL (`generateMonthPlan('practicas')`), agrupa tareas por tipo con conteo real y agrega `explicacion` de por qué se repite (mecanización) vía `REPETICION_POR_TIPO`; `SEMANA_MODULO` mapea semana→módulo. `evaluatePracticaPrueba(moduleId, answers)` evalúa índices y devuelve aciertos/score/aprobado/feedback por pregunta. NPC nuevo `capacitador` en `worldBible.ts` (company `lno`, route `contable`, cumple `npcAuthorized` de story-coherence). Endpoints en `simEngine.ts`: `GET /api/sim/practicas/tracker`, `GET /api/sim/practicas/cursos`, `GET /api/sim/practicas/curso/:id`, `POST /api/sim/practicas/prueba/:id`. Frontend: `PracticasTracker.tsx` (NUEVO, tarjetas por semana con repeticiones ×N, checkbox de dominio, botón "Hacer tarea" y "Ver curso"); `PracticasCurso.tsx` (NUEVO, visor tipo chat con avatar 🎓 del capacitador, burbujas, puntos clave, navegación anterior/siguiente y progreso); `PracticasModules.tsx` gana tabs "📚 Módulos / 📅 Tracker / 🎓 Curso" (prop `initialTab`), quiz interactivo al final de cada módulo con feedback por pregunta y "Reintentar", persistencia en `localStorage('practicas_prueba_results')`; `DesktopShell.tsx` agrega apps "📅 Tracker" y "🎓 Curso" + screens `practicasTracker`/`practicasCurso`. Tests `tests/practicas-modules.test.ts` ampliado a 32 (tracker 4 semanas coherente con plan real y repeticiones de business_expense en semana 2, cursos con NPC capacitador + worldBible, prueba acierta/reprueba, IVA 16% no 10%, propina no deducible, audit ampliada valida cursos/pruebas/tracker). Verificado end-to-end vía API: tracker 4 semanas, curso mod-gastos con 3 secciones, prueba 100% aprobada / 0% reprobada. Regla de oro: conteos de repetición salen del plan real, respuestas correctas son pedagógicas estáticas (no IA), la validación de tareas reales NO se toca, el capacitador es narrativa (no afecta motores). Gates: root **267** tests / audit **106** / backend tsc / builds alumnos y staff limpios.

**R-14 — Integración práctica realista + correcciones 26-ago-2026 (INC-001/002, guías dedicadas, comprobación de tarea)**: ciclo contable completo coherente (documento real → portal → asiento). Backend: documentGenerator (8 docs, persistentData+simTime), rng mulberry32, auditDocument (9 checks), cfdiGenerator (XML 4.0+PDF), workflowDoc/DualViewLayout/DataHighlight (vista dual, pistas mapeo ocultas por defecto como Guía, doc der. ticket La Parrilla LPN-880707-ABC). Fix my-profile select * (practicas), SimuladorLaboral checkOnboarding specialty DB + selectedJob sync. Fix business_expense: ticket real + 9 filas editables (Empresa/RFC/Folio/Subtotal/Propina/IVA/Total/Deducible/Acreditable) + validación real 9 reglas (antes auto-aprobaba). Fix payroll: ISR tarifa progresiva (tramos 0-6k:0, 6k-30k:115.2+6.4%, 30k+:1651.2+10.88%) no 15% fijo, 19 filas editables por empleado (bruto/ISR/IMSS/neto + totales). Fix bank_reconciliation/cash_cut: editables + validación real. Guías dedicadas por cada campo en los 6 módulos (invoice 9, payment 5, supplier 7, business_expense 10+ticket-extract, payroll 5, bank 5; GuideBubbles inline dentro de cada panel, split ticketGuides/formGuides). Comprobación de tarea en tests/practicas-modules.test.ts: módulo↔actividad, no-auto-aprueba, payroll tarifa. Suite 282 tests, tsc 0, builds limpios. Verificado prod: /api/sim/my-profile→practicas/Practicante, payroll ISR tarifa (Ana 1331, Carlos 1869, María 1523, Roberto 2195), bank/cash editables, frontend BOqFr3Pz inline.

**R-15 — Carrera Data completa: 7 motores avanzados (26-ago-2026)**: cierra la carrera data (analista→ingeniería/ciencia) de 12 a 19 capacidades. ackend/src/services/advancedDataEngines.ts (NUEVO): motores reales + 7 validadores AdvancedValidatorId (excel/dax/forecast/automation/llm_api/agent/prompt) + 7 workflows (excel_advanced, powerbi_dax, forecast_sales, automation_etl, llm_integration, agent_task, prompt_engineering) con tipo dvanced. Golden real: DAX total = dbtCatalog.MART_TOTAL=128350, pronóstico serie [112400,118900,124150,128350] con media móvil + MAPE. Router workflows.ts: dvancedTypes + case dvanced en /validate. engineCapabilities.ts: los 7 pasan a exists (quedó solo erp missing); utomator-routes.test actualizado (los construidos ya no van a backlog, 
egisterEngineRequirement dedupe usa erp). 	askPlanner.ts: ADVANCED_WEEKS (analyst→BI/DAX/Excel/forecast, engineering→n8n/LLM/agent, science→prompt) fusionadas en generateMonthPlan por fase; specialties.ts registra los 7. Frontend: PowerBISim.tsx, ForecastSim.tsx, AutomationSim.tsx, AgentSim.tsx, PromptSim.tsx (NUEVOS) + 
enderTool cases + apps por fase (analyst +Power BI/Pronóstico, engineering +Automatización/Agente, science +Pronóstico/Prompt) + DE_SLOTS/ANALYST_SLOTS/DS_SLOTS ampliados. Tests 	ests/advanced-data-engines.test.ts (13). Verificado prod: capabilities exists, powerbi_dax valida 20/20. Suite 294 tests / audit 106 / builds limpios.

**R-15 Capa 0 + Ecosistema (26-ago-2026)**: estructura 3×2 de la carrera data: **Capa 0** = fundamentos por herramienta (mini-módulos, cierre por criterio, NO countsAsCase) → **Capa 1** = ecosistema (herramientas juntas, countsAsCase) → Capstone, por cada rama DA/DE/DS. ackend/src/services/fundamentals.ts (NUEVO): 12 workflows *_basico (DA excel/sql/catalog/bi, DE python/foundry/airflow/git/monitor, DS stats/ml/metricas) + 3 ecosistema_* (da/de/ds) con tool embebida y validadores por tipo; FUNDAMENTAL_TYPES/getFundamentalWorkflow. deValidation.ts: nuevos validadores i (validateBI: visual+origen) y asic_read (identificación de lectura) en DEValidatorId. 	askPlanner.ts: FUNDAMENTALS_WEEKS/ECOSYSTEM_WEEKS/ADVANCED_WEEKS por rama, fusionadas con merge() (no sobrescriben semana) en generateMonthPlan; 15 templates nuevos; Capa 0 no countsAsCase, ecosistema sí. workflows.ts: undamentalsTypes en GET + fallback /validate. simEngine.ts: 	ask-plan/	oday-tasks/week-tasks leen ?route (antes ignorado). progressTracker.ts: sims.total/cases.total derivados del plan real (no hardcodeados 8/3); countsAsCase persistido (DesktopShell→progress/record→
ecordCompletion). specialties.ts: registra los 15. Frontend: ECOSYSTEM_SLOTS/ANALYST_SLOTS/DS_SLOTS ampliados + 
enderTool para stats/ml/monitor; SpreadsheetSim gana XLOOKUP/BUSCARX/SUMIFS/COUNTIFS/UNIQUE/FILTER. docs/simulab-v2-guess-encargado-almacen.json (SIMULAB v2: c/buildPlan densidad 0.4/0.2/0.15/0.15/0.1, cierre por criterio, oral 5m; Bloque 6 VBA→Modelo de Datos Power Pivot/DAX). docs/plan-capas-0-ecosistema.md IMPLEMENTADO (9 gaps resueltos). docs/plan-qa-produccion-capa0.md (plan QA en producción con Chrome DevTools, bloques A-H). Tests 	ests/capa-zero.test.ts (6). Suite 301 tests / audit 106 / builds limpios. Verificado prod: 	ask-plan?route=analyst lista fundamentos→ecosistema; excel_basico/i_basico validan 10/10.

**R-15 Refinamiento Capa 0 Excel (26-ago-2026)**: undamentals.ts Excel pasa de 1 a 3 tareas concretas (1 carga→Power Query, 2 XLOOKUP/BUSCARX, 3 SUMIFS/SUMAR.SI.CONJUNTO) para no ser lienzo en blanco; validación 3 reglas concept (excel). Frontend: DesktopShell pestaña retráctil de tareas dentro del Excel (	ool con 
elative + 	askDrawerOpen, lista Pasos de este ejercicio 1./2./3. + Pendientes del día) y SpreadsheetSim ROW_HEIGHT 24→19px (más compactas). Suite 303 tests.

**P0 TPM consolidado (03-sep-2026)**: baseline audit `20260903_baseline_audit.sql` reescrita ASCII puro + 7 tickets squash RESUELTOS con verificación remota (sim_world 3 rows, sim_progress 1 row, profiles.specialty en uso, sim_world.career_path jsonb + breakdown en state, cv_profiles, account_requests.career_branch, verification_links 1 row; L08→R1, L09→R2, L10→R3; R5/R6 = arboles_remotos 1 row, DDL local pendiente). Hallazgo seguridad: `arboles_remotos` con RLS DISABLED pese a R6 (no auto-remediar, decide Angel). Bucle AND Data Analyst CONGELADO por Angel: `anio` = concepto de limpieza sin-eñe (SQL no maneja la eñe → columna `anio`); avance por compuerta AND de 2 variables (1. pasa tests de calificación de resultados + 2. alumno se siente listo), NO por racha; ejemplo Contalink = `mod-cfdi` con el mismo gate (tests actividad + dominio). `deValidation.ts` gana `validator_estructural` (`validateEstructural`: sin-eñe + requireAll + forbid + seed determinista, `EstructuralRule`) cableado en `runDEValidator`; tests `tests/validator-estructural.test.ts` (6: eñe falla, anio pasa, hueco concept-vs-estructural, forbid, no-vacío, determinismo).

**P1-4 — Backlog futuro (05-sep-2026)**: R-10/R-11/R-12 pasan a `docs/backlog-futuro.md` (pausa autorizada, codigo y tests intactos, nada se borra). Conservado: `piiScrubber.ts` + `POST /api/sim/telemetry` (minimo R-11) y `engineCapabilities.ts` como registro vigente (19 exists, solo `erp` missing). Reactivacion solo por ciclo Gate.
...
**Modo inmersivo en sims (15-sep-2026)**: banner solo en escritorio, header colapsable con hover, +44px de alto útil (`DesktopShell.tsx`, `SimuladorLaboral.tsx`, `StudentPanel.tsx`).

**Sims ContaLink con diseño propio (16-sep-2026)**: `NominaSim`/`ConciliacionSim`/`AuditoriaSim` con diseño ContaLink (heroes por módulo, stats vivas de goldens, 4 fases, teoría puente 899, alta con validación RFC 13 / CURP 18 / NSS 11).

**DIOT único y determinista (16-sep-2026)**: seed por fase + folio de acuse determinista en `alumnos/public/sims/diot.html`; `DIOTSim.tsx` ajustado; `tests/diot-unificado.test.ts` evalúa el HTML de producción.

**Sims reportan progreso al servidor (16-sep-2026)**: `alumnos/src/sims/reportarSim.ts` (NUEVO) — puente Sim → `POST /api/sim/progress/record` (`category: 'sim'`, `specialty: 'practicas'`, `countsAsCase: true`, best-effort sin bloquear el juego); DIOT reporta `DIOT_COMPLETADO` desde el iframe vía `ContalinkFrame.tsx`. Tests `tests/reportar-sim.test.ts`. Alimenta tracker, expediente y trampas.

**Honestidad fiscal docente (16-sep-2026)**: IMSS 5% fijo declarado como simplificación didáctica (la LSS real cotiza por ramos de seguro, comentario en `nominaEngine.ts` + nota visible en el Sim); cuenta 899 = puente interna transitoria, NO se declara en Anexo 24 (`ConciliacionSim.tsx`); teoría cita ISR Art. 96 LISR por tarifa progresiva (nunca % fijo) y cuotas obrero-patronales LSS.

**Pilotos automáticos TourSim (16/17-sep-2026)**: `TourSim.tsx` + `toursContalink.ts` (NUEVOS) — tours individuales por Sim con navegación + gate; el piloto NO genera acuse (solo práctica/examen) y la fila de acuse queda oculta en piloto, visible al presentar (`diot.html`). Tests `tests/tours-sims.test.ts`.

**Shell ContaLink OS unificado claro (17-sep-2026)**: `ContalinkShell.tsx` (NUEVO) — chrome común claro clon de `mas.html` para los 4 submódulos: header con logo, sidebar (Principal / Módulos Contables / Fiscal + buscador Anexo 24 decorativo), breadcrumb + usuario "Angel Aramiz · Practicante", tema CLARO siempre; DIOT monta el iframe existente (`ContalinkFrame` src `/sims/diot.html`).

**Pedagogía practicantes P1+P2+P3 (18-sep-2026)**: **P1 lecciones** — todos los mensajes de error de los 4 engines (`conciliacionEngine`, `nominaEngine`, `diotEngine`, `auditoriaEngine`) llevan lección `📚` que explica el porqué contable/fiscal. **P2 vistas previas** — DIOT `previewModal` del acuse (periodo, totales, folio + sello determinista de 32 hex vía `selloAcuse`, confirmar/revisar) y Nómina `vistaPreviaCFDI` (requiere receptor con RFC 13 del paso Alta). **P3 Caso Real** — `ConciliacionSim` modo datos sucios: `CASO_REAL_FILAS` (6 filas fijas y deterministas: folio con dígito de más, RFC cambiado, centavo de diferencia, cheque en tránsito, 2 limpias) + `resolverCasoReal` (decisiones Corregir/En tránsito/Eliminar; eliminar partida = error grave; falsos positivos penalizan) que reporta progreso al servidor. Tests nuevos: `caso-real`, `lecciones-errores`, `sims-cableado-ui`, `vista-previa-cfdi`. Suite: **523 tests / 63 archivos, todo verde**.

**Motor de pólizas Anexo 24 + 5º Sim Contalink (20-sep-2026, mergeado a `main` vía `feat/poliza-v2` el 22-sep-2026 como `a2e9dcd`)**: del CFDI al asiento en 5 etapas. `backend/src/services/satCatalog.ts` (NUEVO): subconjunto operativo del DOF (36 cuentas con naturaleza, equivalencias cuenta interna→agrupador incl. caso real 601-83→601.45, bancos G, monedas F, métodos H, reglas producto→agrupador, `auditSatCatalog`) — proyección backend del `catalogoAgrupador.ts` del front (patrón dbtCatalog). `backend/src/services/polizaEngine.ts` (NUEVO): ingesta CFDI+EDOCTA → conciliación monto+fecha (tolerancia 0.01) → clasificación (sin regla = sin clasificar, nunca adivina) → cálculo (PUE = egreso con 118.01; PPD = provisión con 119.01 + 201.01; ISR arrendamiento PF 10% Art. 116; ingresos/capital por naturaleza acreedora; gate debe=haber) → póliza formato sección C + balanza sección B; todo error trae lección 📚; semillas MARCELO F 70900/7090/63810, PPD 1000/160/1160, ventas 26680/23000/3680, capital 50000. Endpoints `GET /polizas/catalogo`, `POST /polizas/generar|guardar` (valida cuadratura + agrupadores, 422 si descuadra; acumula en diario, actualiza saldos vía `ensureSatAccount`, persiste en tabla `sim_polizas` — migración aplicada), `GET /polizas`. `Account.agrupador` + `JournalEntry.agrupador/uuid`; `generatePolizaPracticaWorkflow` + `poliza_practica` en accountingTypes/specialties. Frontend: `PolizaSim.tsx` (clon captura: editor multilínea + buscador Anexo 24 + totales con gate + notas UUID + Guardar/XML/PDF + balanza viva, 4 fases, 4 casos semilla, tour 6 pasos, reporta `poliza_practica`), 5º módulo en shell/sidebar/DesktopShell/`simsContalink`, `mod-polizas` contalink (prueba + curso capacitador). Verificado contra backend real: generar → EGRESOS 3 líneas 70900=70900; guardar → folio + diario con agrupador/uuid; descuadrada → 422. Tests `poliza-engine.test.ts` (24). Suite: **563 root / 106 audit, tsc back+alumnos y builds limpios**.

**Rediseño pólizas con experto contable (21-sep-2026, mergeado a `main` vía `feat/poliza-v2` el 22-sep-2026 como `a2e9dcd`; backend limpio `4bb299b` en rama `feat/motor-poliza`)**: subagente planner con rol de contador mexicano auditó motor+Sim y entregó especificación (PF/PM, PPD-ingresos, traspasos, ret IVA, 601.83, cheques, USD, redondeo, 12 BLOQUEA). Implementado: `buscarCuentas` (backend `satCatalog` + espejo `buscarCuentasFront` con idéntica normalización/ranking: código 100 > prefijo 85 > token 70 > sinónimo+20 > rubro 20; `SINONIMOS_CUENTA` con jerga del alumno) — el Sim ahora busca por línea **por nombre** ("renta" → 601.45+601.46 rankeados con [Usar] que inserta la interna y chip del agrupador + hint →DEBE/HABER), banner de colisión 601.83 vs 601-83, `desambiguarArrendamiento` por RFC (13→601.45, 12→601.46; PM test golden), ingresos PPD → 209.01 (no 208.01), bloqueo 601.83+IVA, `validarUuid/validarRfc` en motor y `/guardar` (UUID duplicado consultando memoria+Supabase, RFC formato, PPD≠EGRESOS, PUE sin conciliar≠EGRESOS, 601.83+118/119), semillas UUID v4 válidas, quiz +2 (PM sin retención, trampa guion/punto) y 4ª sección del curso, workflow email con naturalezas. Hallazgo de la prueba web: el catálogo front no conocía las internas del curso (601-83/216-03/102-01-002) y bloqueaba Guardar — corregido con regresión en `catalogo-agrupador.test.ts`. Verificado en Chrome real contra backend local: onboarding→prácticas→Contalink 5 módulos→Pólizas, generar MARCELO (EGRESOS 70900=70900), guardar→folio+reporte+balanza viva, PPD→PROVISIÓN 1000/160/1160, búsqueda "renta"/"no deducible"→[Usar] inserta + banner colisión. Tests `busqueda-cuentas.test.ts` (12, incl. paridad front/back) + supertest fiscal (duplicado/RFC/PPD/601.83). Suite: **595 root / 106 audit / 93 backend, tsc y builds limpios**. Nota 22-sep: la rama nació contaminada con `exp/finanzas` — se reconstruyó limpia (`f182d32` base + `4bb299b` rediseño, cero finanzas) y se mergeó vía `feat/poliza-v2`; `main` limpio en `3011514` antes del merge; ramas tainted borradas; `exp/finanzas` vive solo en su rama. Backend mergeado a su `main` como `6997daf` (pusheado 22-sep, prod build `6997daf` verificado vía `/api/health`); front 22-sep bundle `index-3CkrR7L2.js`+`SimuladorLaboral-BDiz6OP2.js` con `sim-poliza`/`601-83` y sin `FinanzasDemo`.

**Pólizas kinder + dataset 21 operaciones (22/23-sep-2026, ramas `feat/poliza-kinder` + `feat/poliza-dataset`, sin merge)**: rediseño didáctico validado por subagentes contador-educador y diseñador — 1+2 fusionado (📄 papel vs 🐖 alcancía), regla de oro + columpio visual, 🕵️ Detective 3.5 con protocolo de descuadre y tarjeta 601.83 (hospital/no basurero + declaración), balanza por rubros Balance/Resultados con GRAN TOTAL=ΣDEBE−ΣHABER, piloto kinder hazlo-por-mí/yo-lo-intento. Dataset `backend/src/data/polizaDataset.ts` (21 filas CFDI↔banco: MARCELO golden + diesel/fletes/rentas/taller/oficina/viáticos/honorarios/ventas/capital jun-jul 2026; PUE con banco mismo mes/mismo total, PPD promesa sin banco; UUID=id único) + tabla `poliza_casos` (migración con auto-siembra) + servicio `polizaCasos.ts` (Supabase con fallback memoria) + endpoints `GET /polizas/casos` y `GET /polizas/semilla?exclude=` (sin repetir, 404 agotadas). PolizaSim carga el catálogo con fallback local + botón 🎲 Otra semilla (usadas en localStorage). Hallazgos de prueba web Chrome: GRAN TOTAL sumaba saldos con signo ($14,180 falso → ΣDEBE−ΣHABER, `c52b640`) y guardar ignoraba el agrupador del motor (diesel iba a 601.45 → prefiere `l.agrupador`, edición manual lo limpia). Tests `poliza-dataset.test.ts` (11: volumen/UUID/RFC, aritmética, PUE↔banco, motor genera las 21 cuadradas, diesel→601.48, semilla determinista) + supertest casos/semilla. Suite: **611 root / 106 audit / 95 backend, tsc y builds limpios**.

**Merge a main 24-sep-2026 (`fe4200a`)**: kinder + dataset + loop tester (root) y dataset + endpoints (backend `5e1bad5`). Tester-estudiante consolidado como rol recurrente en `roles/tester-estudiante.md` (4 corridas: 8/5/6/4 → 8/9/8/7). Suite al merge: **624 root / 106 audit / 95 backend**. Pendiente manual: aplicar `supabase/migrations/20260923090000_poliza_casos.sql` en Supabase prod (sin ella el servicio usa el dataset en memoria: funciona igual, sin persistencia compartida).
