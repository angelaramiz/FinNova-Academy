...
export default function DesktopShell({ theme, tasks, onClose, onTaskComplete }: DesktopShellProps) {
  ...
  async function startTask(task: TaskInfo, skipEmailStep = false) {
    ...
    try {
      ...
      setWorkflow(wf);
      // Si la tarea tiene email y NO se debe saltar, empezar en email (paso 0)
      // Si se debe saltar o no hay email, empezar en el paso con contenido real (índice 1)
      const firstContentStep = wf.steps.findIndex((s: any) => s.type !== 'email');
      setStepIdx(skipEmailStep ? firstContentStep : 0);
      setSkipEmail(skipEmailStep);
  ...
  function closeWorkflow() { setScreen('desktop'); setCurrentTask(null); setWorkflow(null); setStepIdx(0); setValidationResult(null); setSkipEmail(false); }

  function openTaskFromEmail(taskId: string) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-3 py-2 border-b-2 flex items-center justify-between shrink-0" style={{ borderColor: colors.border, background: isDark ? '#1a1a2e' : '#e5e7eb' }}>
        ...
        <div className="flex items-center gap-2.5 text-[8px] font-mono" style={{ color: colors.textMuted }}>
          ...
          <button onClick={onClose} className="w-5 h-5 rounded flex items-center justify-center text-[9px] cursor-pointer hover:opacity-70" style={{ background: '#ef4444', color: '#fff' }}>✕</button>
        </div>
      </div>
        ...
        {screen === 'desktop' && (
          <div className="h-full p-6 overflow-auto">
            <div className="flex gap-6 mb-8 flex-wrap">
              {appIcons.map((app, i) => (
                <div key={i} className="flex flex-col items-center gap-1.5 w-16 cursor-pointer hover:opacity-80 transition" onClick={app.action}>
                  <div className="w-14 h-14 rounded-xl border-2 flex items-center justify-center text-lg" style={{ borderColor: colors.border, background: colors.cardBg, boxShadow: `3px 3px 0px 0px ${colors.border}` }}>
                    {app.icon}
                  </div>
                  <span className="text-[8px] font-bold font-mono text-center leading-tight" style={{ color: colors.text }}>
                    {app.label}
                    {app.count && app.count > 0 && app.label !== 'Tareas' ? <span className="ml-0.5" style={{ color: colors.primary }}>({app.count})</span> : null}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-xl border-2 overflow-hidden" style={{ borderColor: colors.border }}>
              <div className="px-4 py-2.5 border-b-2 text-[10px] font-bold font-mono" style={{ borderColor: colors.border, background: isDark ? 'rgba(0,0,0,0.3)' : colors.bg, color: colors.text }}>📋 Pendientes del día</div>
    ...
    <div className="h-full flex flex-col" style={{ background: colors.bg }}>
            <div className="px-4 py-3 border-b-2 shrink-0 flex items-center gap-2" style={{ borderColor: colors.border, background: isDark ? 'rgba(0,0,0,0.4)' : colors.bg }}>
              ...
              <span className="text-[8px] font-mono ml-auto" style={{ color: colors.textMuted }}>{tasks.filter(t => t.type !== '').length} documentos</span>
            </div>
            <div className="flex-1 overflow-auto p-4 space-y-2">
              {(tasks.length === 0) ? (
                <p className="text-[10px] text-center py-8" style={{ color: colors.textMuted }}>No hay documentos archivados</p>
              ) : (
                tasks.map((t, i) => (
                  <div key={t.id} className="flex items-center gap-3 p-3 rounded-xl border-2" style={{ borderColor: colors.border, background: colors.cardBg }}>
                    ...
                    <button onClick={() => startTask(t, true)} className="text-[8px] px-2 py-1 rounded-lg border-2 font-bold cursor-pointer shrink-0" style={{ borderColor: colors.primary, color: colors.primary, background: 'transparent' }}>Abrir</button>
                  </div>
                ))
              )}
            </div>
