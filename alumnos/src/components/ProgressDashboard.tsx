  ...
  if (!progress) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4" style={{ background: colors.bg }}>
        ...
        <button onClick={onBack} className="text-[13px] px-4 py-2 rounded-lg border-2 cursor-pointer" style={{ borderColor: colors.border, color: colors.textMuted }}>← Volver</button>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col" style={{ background: colors.bg }}>
      ...
      <div className="px-4 py-3 border-b-2 shrink-0 flex items-center gap-3" style={{ borderColor: colors.border, background: isDark ? '#0f172a' : '#f8fafc' }}>
        ...
        <div className="flex-1">
          ...
          <span className="text-[11px] font-mono ml-2" style={{ color: colors.textMuted }}>Julio 2026</span>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg" style={{ background: '#22c55e20' }}>
          <span className="text-[11px]">🔥</span>
          <span className="text-[12px] font-bold font-mono" style={{ color: '#22c55e' }}>{progress.streak} días</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="rounded-xl p-3 flex flex-col gap-1" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
              ...
              <span className="text-[11px] font-mono" style={{ color: colors.textMuted }}>{kpi.label}</span>
        ...
        <div className="rounded-xl p-4" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[13px] font-bold" style={{ color: colors.text }}>Progreso del mes</span>
            <span className="text-[13px] font-mono font-bold" style={{ color: colors.primary }}>{completionPct}%</span>
          </div>
          ...
          <div className="flex justify-between mt-1">
            <span className="text-[13px] font-mono" style={{ color: colors.textMuted }}>0%</span>
            <span className="text-[13px] font-mono" style={{ color: colors.textMuted }}>{progress.completedTasks} de {progress.totalTasks} tareas</span>
            <span className="text-[13px] font-mono" style={{ color: colors.textMuted }}>100%</span>
          </div>
        </div>

        {/* Weekly Calendar */}
        <div className="rounded-xl p-4" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
          <div className="text-[13px] font-bold mb-3" style={{ color: colors.text }}>📅 Calendario semanal</div>
              ...
              <div key={i}>
                <div className="flex items-center justify-between mb-1 cursor-pointer" onClick={() => setSelectedWeek(selectedWeek === i ? null : i)}>
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] font-bold" style={{ color: colors.primary }}>S{week.week}</span>
                    <span className="text-[11px]" style={{ color: colors.textMuted }}>{week.theme}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-mono" style={{ color: week.completed > 0 ? '#22c55e' : colors.textMuted }}>{week.completed}/{week.total}</span>
                    {week.trapDetected > 0 && <span className="text-[13px] px-1 rounded" style={{ background: '#f59e0b20', color: '#f59e0b' }}>⚠{week.trapDetected}</span>}
                  </div>
                </div>
                {/* Week progress bar */}
        ...
        {/* Category Breakdown */}
        <div className="rounded-xl p-4" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
          <div className="text-[13px] font-bold mb-3" style={{ color: colors.text }}>📊 Por categoría</div>
          <div className="space-y-2">
            {Object.entries(progress.byCategory).filter(([_, v]) => v.completed > 0 || v.total > 0).map(([cat, data]) => (
              <div key={cat} className="flex items-center gap-2">
                <span className="text-[11px] w-24 truncate" style={{ color: colors.textMuted }}>{CATEGORY_LABELS[cat] || cat}</span>
                ...
                <span className="text-[11px] font-mono w-12 text-right" style={{ color: colors.text }}>{data.completed}/{data.total}</span>
                <span className="text-[11px] font-mono w-8 text-right" style={{ color: getScoreColor(data.avgScore) }}>{data.avgScore}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl p-4" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
          <div className="text-[13px] font-bold mb-3" style={{ color: colors.text }}>🎯 Por dificultad</div>
          <div className="grid grid-cols-4 gap-2">
            {[
              { level: 1, label: '🌱 Básico', color: '#22c55e' },
              { level: 2, label: '📈 Intermedio', color: '#f59e0b' },
              { level: 3, label: '🔴 Avanzado', color: '#f97316' },
              { level: 4, label: '🎯 Experto', color: '#ef4444' },
            ].map(d => {
              ...
              return (
                <div key={d.level} className="rounded-lg p-2 text-center" style={{ background: isDark ? '#0f172a' : '#fff', border: `1px solid ${colors.border}` }}>
                  <div className="text-[11px] mb-1">{d.label}</div>
                  <div className="text-sm font-bold font-mono" style={{ color: d.color }}>{data.completed}/{data.total}</div>
                  <div className="text-[13px] font-mono" style={{ color: colors.textMuted }}>{data.avgScore}%</div>
                </div>
              );
            })}
          </div>
        ...
        {/* Recent Activity */}
        <div className="rounded-xl p-4" style={{ background: isDark ? '#1a1a2e' : '#f8fafc', border: `1px solid ${colors.border}` }}>
          <div className="text-[13px] font-bold mb-3" style={{ color: colors.text }}>🕐 Actividad reciente</div>
          <div className="space-y-2">
            {progress.recentCompletions.length === 0 ? (
              <div className="text-center py-4 text-[12px]" style={{ color: colors.textMuted }}>Sin actividad aún</div>
              ...
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg" style={{ background: isDark ? '#0f172a' : '#fff', border: `1px solid ${colors.border}` }}>
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold" style={{ background: t.passed ? '#22c55e20' : '#ef444420', color: t.passed ? '#22c55e' : '#ef4444' }}>
                  {t.passed ? '✓' : '✗'}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] font-mono font-bold truncate" style={{ color: colors.text }}>{t.title}</div>
                  <div className="text-[13px] font-mono" style={{ color: colors.textMuted }}>S{t.week}D{t.day} · {t.timeSpent}min</div>
                </div>
                <span className="text-[11px] font-mono font-bold" style={{ color: getScoreColor(t.score) }}>{t.score}%</span>
              </div>
            ))}
          </div>
        </div>
