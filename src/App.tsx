...
function AppContent() {
  ...
  return (
    <div id="finance-platform-core" className="min-h-screen bg-[#0a0f1d] text-slate-300 flex flex-col font-sans selection:bg-teal-450 selection:text-neutral-900">
      ...
      {/* -------------------------------------------------------------
          PERSPECTIVE SWITCHER SANDBOX PANEL (THREE ROLES)
          ------------------------------------------------------------- */}
      <div 
        id="sandbox-control-panel"
        className="fixed bottom-4 right-4 z-[99] bg-slate-900/95 border border-slate-800/60 p-3 rounded-2xl shadow-xl max-w-xs flex flex-col gap-2 backdrop-blur-md"
      >
        <div className="flex items-center gap-1.5 text-[9px] font-semibold uppercase text-slate-400 tracking-wider font-mono">
          <Sliders className="w-3 h-3 text-slate-400" /> Sandbox Controller
        </div>
        <div className="grid grid-cols-3 gap-1">
          <button
            onClick={() => handleSandboxRoleSwitch('student')}
            className={`py-1 px-1.5 rounded-lg text-[9px] font-bold transition cursor-pointer border ${
              currentViewMode === 'student'
                ? 'bg-teal-500/10 border-teal-500/30 text-teal-400 shadow-sm'
                : 'bg-slate-950/30 border-slate-850/60 text-slate-500 hover:text-slate-300'
            }`}
          >
            Alumno
          </button>
          <button
            onClick={() => handleSandboxRoleSwitch('instructor')}
            className={`py-1 px-1.5 rounded-lg text-[9px] font-bold transition cursor-pointer border ${
              currentViewMode === 'instructor'
                ? 'bg-teal-500/10 border-teal-500/30 text-teal-400 shadow-sm'
                : 'bg-slate-950/30 border-slate-850/60 text-slate-500 hover:text-slate-300'
